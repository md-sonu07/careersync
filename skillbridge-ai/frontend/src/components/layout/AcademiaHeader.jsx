...
export default function AcademiaHeader({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[68px] items-center gap-3 border-b border-border bg-white/80 backdrop-blur-md supports-[backdrop-filter]:bg-white/70 px-4 sm:px-6 shadow-sm">
      ...
      <div className="hidden lg:flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold leading-none text-charcoal">Academia Intelligence</p>
          <p className="text-xs text-muted">1,248 students • 78% placed</p>
        </div>
        <span className="ml-2 hidden items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2.5 py-1 text-xs font-bold text-amber-700 xl:inline-flex">2 gaps critical</span>
      </div>
      <div className="flex flex-1 items-center justify-center px-2 sm:px-4 lg:px-8">
        <div className="relative w-full max-w-[560px]">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search students, skills, reports…" className="w-full rounded-full border border-border bg-background pl-10 pr-[88px] py-2.5 text-sm text-charcoal placeholder:text-muted/60 shadow-soft focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-colors" />
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-80 overflow-hidden rounded-2xl border border-border bg-white shadow-card">
              ...
              <div className="divide-y divide-border">
                <div className="flex gap-3 p-3 hover:bg-background"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-100 text-amber-700 shrink-0"><span className="material-symbols-outlined text-[18px]">warning</span></span><div><p className="text-sm font-medium text-charcoal">Docker gap critical 44%</p><p className="text-xs text-muted">Needs workshop • 1h ago</p></div></div>
                <div className="flex gap-3 p-3 hover:bg-background"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-primary shrink-0"><span className="material-symbols-outlined text-[18px]">trending_up</span></span><div><p className="text-sm font-medium text-charcoal">Placement rate +5%</p><p className="text-xs text-muted">78% current • 2h ago</p></div></div>
              </div>
              <Link to="/academia/notifications" onClick={() => setNotifOpen(false)} className="block border-t border-border bg-background px-4 py-2.5 text-center text-sm font-semibold text-primary hover:bg-sage">View all →</Link>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2.5 rounded-full border border-border bg-white pl-1.5 pr-1.5 py-1 shadow-soft sm:pr-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white text-xs font-bold shrink-0">{displayName.slice(0, 2).toUpperCase()}</div>
          <div className="hidden text-left sm:block">
            <p className="text-sm font-semibold leading-none text-charcoal">{displayName}</p>
            <p className="text-xs leading-none text-muted">Academia</p>
          </div>
