...
export default function IndustryInternships() {
  ...
  const loadInternships = async () => {
    try {
      setLoading(true)
      const data = await opportunityApi.getOpportunities({ type: 'internship', my_posts: 'true' })
      if (Array.isArray(data)) {
        setInternships(data.filter((opp) => !opp.opportunity_type || opp.opportunity_type === 'internship'))
      }
    } catch (err) {
  ...
  const handleEditClick = (item) => {
    ...
    setEditForm({
      ...
      stipend_salary: item.stipend_salary || '₹15,000/month',
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error('Failed to delete internship post.')
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-charcoal">Manage Posted Internships</h1>
          <p className="text-sm text-muted">View, edit, update and manage all internship listings posted by your company.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={loadInternships} disabled={loading}>
            <AppIcon name="refresh" className="text-[16px]" /> Refresh
          </Button>
          <Link to="/industry/internship/new">
            <Button variant="primary" size="sm" className="font-bold flex items-center gap-1">
              <AppIcon name="add" className="text-[18px]" /> Post New Internship
            </Button>
          </Link>
        </div>
      </div>
      ...
      <Card className="p-4 flex flex-col sm:flex-row items-center gap-3">
        <div className="flex-1 w-full">
          <SearchInput value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search internships by role, location, or keyword..." />
        </div>
      ...
      {loading ? (
        <Card className="p-12 text-center text-muted">
          ...
          Loading your company's internship postings...
        </Card>
              ...
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-xl bg-background border border-border px-3 py-2 flex items-center gap-1.5">
                    <AppIcon name="schedule" className="text-[16px] text-muted" /> {item.duration || '3 months'}
                  </div>
                  <div className="rounded-xl bg-background border border-border px-3 py-2 flex items-center gap-1.5 font-semibold">
                    <AppIcon name="payments" className="text-[16px] text-muted" /> {item.stipend_salary || '₹15,000/month'}
                  </div>
                </div>
        ...
        <Card className="p-12 text-center text-muted">
          <AppIcon name="work_outline" className="text-4xl text-primary mx-auto mb-2" />
          <h3 className="text-lg font-bold text-charcoal">No Internship Postings Found</h3>
          <p className="text-sm text-muted mt-1 max-w-md mx-auto">You haven't posted any internships matching your search yet.</p>
          <Link to="/industry/internship/new" className="inline-block mt-4">
            <Button variant="primary" size="sm">Post New Internship Now</Button>
          </Link>
        </Card>
      )}

      {/* Edit Internship Modal */}
      <Modal open={!!editingItem} onClose={() => setEditingItem(null)} title="Edit Internship Posting" size="lg">
        <div className="space-y-4">
          ...
          <Textarea
            ...
            placeholder="Describe internship role, responsibilities, and requirements..."
          />
      ...
      <Modal open={!!deletingId} onClose={() => setDeletingId(null)} title="Delete Internship Posting" size="sm">
        <div className="space-y-4 text-center py-2">
          <AppIcon name="warning" className="text-4xl text-amber-500 mx-auto" />
          <h4 className="font-bold text-charcoal text-base">Are you sure you want to delete this internship post?</h4>
