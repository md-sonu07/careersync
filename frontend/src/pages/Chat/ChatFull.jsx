          ...
          return (
            <code
              className={`px-1.5 py-0.5 rounded text-xs font-mono font-semibold ${
                theme === 'dark' ? 'bg-[#383838] text-emerald-400' : 'bg-gray-200 text-primary'
              }`}
              {...props}
            >
              {children}
...
export default function ChatFull() {
  ...
  const handleSend = async (textOverride = null) => {
    ...
    try {
      ...
      const response = await aiAPI.sendMessage(textToSend, targetId)
  ...
  return (
    <div
      className={`flex h-screen w-full font-sans overflow-hidden transition-colors ${
        theme === 'dark' ? 'bg-[#212121] text-[#ececec]' : 'bg-[#FCFCFC] text-charcoal'
      }`}
    >
      {/* Sidebar */}
      <div
        className={`border-r transition-all duration-300 flex flex-col ${
          theme === 'dark' ? 'bg-[#171717] border-[#2f2f2f]' : 'bg-[#F9F9F9] border-border/60'
      ...
      >
        <div className="p-4 flex items-center justify-between">
          ...
          <button
            ...
            className={`p-1.5 flex items-center justify-center cursor-pointer rounded-md transition-colors ${
              theme === 'dark'
        ...
        <div className="px-3 pb-2 pt-3">
          <button
            ...
            className={`flex items-center gap-2 rounded-lg font-semibold py-2.5 px-3 cursor-pointer w-full text-sm transition-all shadow-sm active:scale-[0.98] ${
              theme === 'dark'
          ...
          {isLoadingConversations ? (
            ...
            <div
              className={`text-center text-xs mt-6 ${
                theme === 'dark' ? 'text-gray-500' : 'text-muted'
              }`}
            >
            ...
            <div
              className={`text-xs font-semibold uppercase tracking-wider px-2 py-2 mb-1 ${
                theme === 'dark' ? 'text-gray-500' : 'text-muted/60'
              }`}
            >
              Recent
            </div>
                ...
                {editingId === c.id ? (
                  ...
                  >
                    <input
                      ...
                      autoFocus
                      className={`text-xs font-normal border rounded px-1.5 py-0.5 flex-1 focus:outline-none min-w-0 ${
                        theme === 'dark'
        ...
        <div
          className={`p-3 border-t ${
            theme === 'dark' ? 'border-[#2f2f2f]' : 'border-border/60'
          }`}
        >
          {isAuthenticated ? (
            <div className="relative">
              <button
                ...
                className={`w-full flex items-center gap-2.5 p-2 rounded-lg transition-colors ${
                  theme === 'dark' ? 'hover:bg-[#252525]' : 'hover:bg-border/40'
                }`}
              >
                ...
                <div className="text-left truncate flex-1">
                  ...
                  <div
                    className={`text-[10px] truncate capitalize ${
                      theme === 'dark' ? 'text-gray-400' : 'text-muted'
                    }`}
                  >
                    {user?.role || 'Student'}
                  </div>
              ...
              {profileDropdownOpen && (
                <div
                  className={`absolute bottom-full left-0 w-full mb-1 border shadow-lg rounded-xl overflow-hidden py-1 z-50 animate-in fade-in slide-in-from-bottom-2 ${
                    theme === 'dark'
                ...
                >
                  <Link
                    ...
                    className={`block px-4 py-2 text-sm transition-colors ${
                      theme === 'dark' ? 'hover:bg-[#383838]' : 'hover:bg-background'
                    }`}
                  >
                    Dashboard
                  </Link>
      ...
      >
        ...
        <div className="absolute top-0 left-0 w-full p-4 flex items-center justify-between z-10 pointer-events-none">
          <div className="pointer-events-auto flex items-center gap-2">
            {!sidebarOpen && (
              <button
                ...
                className={`p-1.5 flex items-center justify-center cursor-pointer rounded-md backdrop-blur-sm border shadow-sm transition-colors ${
                  theme === 'dark'
            ...
            <div
              className={`flex items-center gap-2 px-3 py-1.5 rounded-md backdrop-blur-sm border shadow-sm cursor-default ${
                theme === 'dark'
          ...
          <div className="pointer-events-auto flex items-center gap-3">
            ...
            <button
              ...
              className={`p-2 rounded-lg cursor-pointer transition-colors flex items-center justify-center border shadow-sm ${
                theme === 'dark'
            ...
            <Link
              to="/"
              className={`text-sm font-medium backdrop-blur-sm px-3 py-1.5 rounded-lg border shadow-sm flex items-center gap-1.5 transition-colors ${
                theme === 'dark'
        ...
        <div className="flex-1 overflow-y-auto w-full pb-36 pt-16 scroll-smooth">
          {!activeConversationId && messages.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-220px)] max-w-3xl mx-auto px-6 text-center animate-in fade-in zoom-in-95 duration-500">
              ...
              <h1
                className={`text-4xl md:text-5xl font-[400] tracking-tight mb-4 ${
              ...
              <p
                className={`text-sm md:text-base mb-8 max-w-md ${
                  theme === 'dark' ? 'text-gray-400' : 'text-[#666666]'
                }`}
              >
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {chips.map((c) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl border text-xs font-medium shadow-xs transition-all cursor-pointer ${
                      theme === 'dark'
                ...
                >
                  {m.role === 'user' ? (
                    <div className="flex items-end gap-3 max-w-[85%] sm:max-w-[75%]">
                      <div
                        className={`rounded-2xl rounded-tr-sm px-4 py-3 text-[14px] leading-relaxed font-medium shadow-xs ${
                          theme === 'dark'
              ...
              {isSending && (
                <div className="flex items-start gap-3 max-w-[85%] sm:max-w-[80%] animate-in fade-in duration-200">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div
                      className={`rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1.5 w-fit ${
                        theme === 'dark'
                          ? 'bg-[#2f2f2f]'
                          : 'bg-[#f4f4f4]'
                      }`}
                    >
                      <div
                        className={`w-2 h-2 rounded-full animate-bounce [animation-delay:-0.3s] ${
                          theme === 'dark' ? 'bg-white' : 'bg-charcoal/70'
                        }`}
                      />
                      <div
        ...
        <div
          className={`absolute bottom-0 left-0 w-full pt-10 pb-6 px-4 ${
            theme === 'dark'
        ...
        >
          <div
            className={`max-w-3xl mx-auto w-full rounded-2xl shadow-lg border transition-all flex flex-col ${
              theme === 'dark'
          ...
          >
            <textarea
              ...
              className={`w-full resize-none bg-transparent px-4 py-3.5 text-[14px] focus:outline-none disabled:opacity-50 ${
                theme === 'dark'
            ...
            <div className="px-3 pb-2.5 flex items-center justify-between">
              ...
              <button
                ...
                className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors shadow-sm cursor-pointer ${
                  theme === 'dark'
