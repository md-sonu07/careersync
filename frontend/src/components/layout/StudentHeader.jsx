    ...
    <header className="sticky top-0 z-20 flex h-[68px] items-center justify-between gap-2 border-b border-border bg-white/80 backdrop-blur-md supports-[backdrop-filter]:bg-white/70 px-3 sm:px-6 shadow-sm">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-3">
          <span className="hidden items-center gap-1.5 rounded-lg bg-sage border border-border px-2.5 py-1 text-xs font-bold text-primary xl:inline-flex shrink-0">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <div className="relative shrink-0">
          <button onClick={() => setNotifOpen((v) => !v)} className="relative inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-lg border border-border bg-white text-charcoal hover:bg-background shadow-soft">
            ...
            <span className="absolute -right-0.5 -top-0.5 flex h-4 sm:h-5 min-w-[16px] sm:min-w-[20px] items-center justify-center rounded-lg bg-primary px-1 text-[10px] sm:text-[11px] font-bold text-white shadow-sm">1</span>
          </button>
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-72 sm:w-80 overflow-hidden rounded-2xl border border-border bg-white shadow-card z-50">
              ...
              <div className="divide-y divide-border">
                <div className="flex gap-3 p-3 hover:bg-background"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 text-primary shrink-0"><AppIcon name="verified" className="text-[18px]" /></span><div className="min-w-0 flex-1"><p className="text-xs sm:text-sm font-medium leading-tight text-charcoal truncate">Welcome to CareerSync</p><p className="text-[10px] sm:text-xs text-muted">Complete your profile to get started</p></div></div>
              </div>
              <Link to="/student/notifications" onClick={() => setNotifOpen(false)} className="block border-t border-border bg-background px-4 py-2.5 text-center text-xs sm:text-sm font-semibold text-primary hover:bg-sage">View all →</Link>
            </div>
          )}
        </div>
